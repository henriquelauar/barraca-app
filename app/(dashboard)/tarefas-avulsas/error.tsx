...
type TarefasAvulsasErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function TarefasAvulsasError({
  error,
  reset,
}: TarefasAvulsasErrorProps) {
  return (
    <div className="rounded-2xl border border-rose-200 bg-rose-50 p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-rose-800">
        Não foi possível carregar as tarefas avulsas
      </h2>

      <p className="mt-2 text-sm text-rose-700">
        Ocorreu um erro ao montar a página de tarefas avulsas.
      </p>
